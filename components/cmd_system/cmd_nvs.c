...
//    list_args.type->sval[0] = "";
    ...
    const char *part = list_args.partition->count == 0 ? "nvs" : list_args.partition->sval[0];
    const char *name = list_args.namespace->count == 0 ? NULL : list_args.namespace->sval[0];
    const char *type = list_args.type->count == 0 ? NULL : list_args.type->sval[0];
...
static int get_stats(int argc, char **argv)
{
    ...
    esp_err_t err = nvs_get_stats(NULL, &s);
    ...
    ESP_LOGI(TAG, "Stats Used:%d Free:%d Total:%d ns:%d", s.used_entries, s.free_entries, s.total_entries, s.namespace_count);

    nvs_handle_t nvs;
    err = nvs_open(current_namespace, NVS_READONLY, &nvs);
    if (err) {
        ESP_LOGE(TAG, "Open Error %s", esp_err_to_name(err));
        return 1;
    }
    
    size_t used_entries;
    err = nvs_get_used_entry_count(nvs, &used_entries);
    if (err) {
        ESP_LOGE(TAG, "Used Entry Error %s", esp_err_to_name(err));
        nvs_close(nvs);
        return 1;
    }
    ESP_LOGI(TAG, "used entries %d", used_entries);
    nvs_close(nvs);
...
void register_nvs(void)
{
    ...
    list_args.partition = arg_str0("p", "part", "<partition>", "partition name");
    list_args.namespace = arg_str0(NULL, NULL, "<namespace>", "namespace name");
